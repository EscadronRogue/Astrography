...
export class EditManager {
  constructor(mollweideMap, cachedStars, constellationLabelsMoll, galacticDirectionLabelsMoll, getStarId, buildAndApplyFilters, maybePersistPresets, requestRender) {
    ...
    this.starLabelOffsets = new Map();
    this.starLabelRotations = new Map();
    this.starLabelScales = new Map();
    this.constellationLabelOffsets = new Map();
    this.galacticLabelOffsets = new Map();
    this.editableLabels = [];
    this.selectedLabel = null;
    this.dragOffset = new THREE.Vector3();
    this.editPointer = new THREE.Vector2();
    this.editRaycaster = new THREE.Raycaster();
    this.editPlane = new THREE.Plane(new THREE.Vector3(0, 0, 1), 0);
    ...
    this.rotateCurrentRotation = 0;
    this.scaleStart = null;

    this.removedLineSegments = new Set();
    this.hiddenLineKeys = new Set();

    this.ROTATE_SENSITIVITY = 0.3;
    this.isolationOverlay = null;
  }

  downloadLabelEdits() {
    const edits = {
      starOffsets: Array.from(this.starLabelOffsets.entries()),
      starRotations: Array.from(this.starLabelRotations.entries()),
      starScales: Array.from(this.starLabelScales.entries()),
      constellationOffsets: Array.from(this.constellationLabelOffsets.entries()),
      galacticOffsets: Array.from(this.galacticLabelOffsets.entries())
    };
  ...
  applyLabelEdits(edits) {
    ...
    if (edits.starOffsets) {
      this.starLabelOffsets.clear();
      edits.starOffsets.forEach(([id, off]) => this.starLabelOffsets.set(id, off));
    }
    if (edits.starRotations) {
      this.starLabelRotations.clear();
      edits.starRotations.forEach(([id, rot]) => this.starLabelRotations.set(id, rot));
    }
    if (edits.starScales) {
      this.starLabelScales.clear();
      edits.starScales.forEach(([id, sc]) => this.starLabelScales.set(id, sc));
    }
    if (edits.constellationOffsets) {
      this.constellationLabelOffsets.clear();
      edits.constellationOffsets.forEach(([id, off]) => this.constellationLabelOffsets.set(id, off));
    }
    if (edits.galacticOffsets) {
      this.galacticLabelOffsets.clear();
      edits.galacticOffsets.forEach(([id, off]) => this.galacticLabelOffsets.set(id, off));
    }

    if (this.cachedStars) {
      this.cachedStars.forEach(star => {
        ...
        if (this.starLabelRotations.has(id)) {
          star.mollLabelRotation = this.starLabelRotations.get(id);
        } else {
  ...
  setupEditIOControls() {
    ...
    if (dlBtn) {
      dlBtn.addEventListener('click', () => this.downloadLabelEdits());
    }
  ...
  getState() {
    return {
      starLabelOffsets: this.starLabelOffsets,
      starLabelRotations: this.starLabelRotations,
      starLabelScales: this.starLabelScales,
      constellationLabelOffsets: this.constellationLabelOffsets,
      galacticLabelOffsets: this.galacticLabelOffsets,
      removedLineSegments: this.removedLineSegments,
      hiddenLineKeys: this.hiddenLineKeys
    };
  }
}
