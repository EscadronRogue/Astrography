...
**Remaining issues**
- the file is still very large and mixes application bootstrap, three map implementations, export logic, edit tooling, persistence wiring, and startup orchestration
- the `state` object is still not a real plain state tree; it is an `Object.defineProperties` compatibility facade over many top-level variables
- there are still many DOM lookups and browser event registrations embedded directly in the file
- the export subsystem and edit subsystem are substantial enough to justify dedicated modules, but they are still embedded here
- several helpers are wrappers around shared helpers and add indirection without much value
- important constants still live locally rather than in shared configuration
- the file still makes maintenance expensive because almost every major feature eventually intersects with it

**Improve**
- replace the property-facade state layer with a real application state object
- move export-selection logic into its own module
- move label/line editing into a dedicated subsystem module
- remove wrapper helpers that only forward to shared helpers
