...
import { createExportManager } from './script/exportManager.js';
import { createEditManager } from './script/editManager.js';
...
class MapManager {
  ...
  updateConnections(stars, connectionObjs, opacity = 0.5) {
    ...
    editManager?.applyStoredLineEdits?.(this.connectionGroup);
    requestRenderIfAvailable();
  }
...
function initializeManagers() {
  exportManager = createExportManager({ mollweideMap });
  editManager = createEditManager({
    mollweideMap,
    getConstellationLabelsMoll: () => constellationLabelsMoll,
    getConstellationLinesMoll: () => constellationLinesMoll,
    getGalacticDirectionLabelsMoll: () => galacticDirectionLabelsMoll,
    getIsolationOverlay: () => isolationOverlay,
    getCachedStars: () => cachedStars,
    requestRender,
    maybePersistPresets,
