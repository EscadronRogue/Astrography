...
import { radToSphere, cachedRadToMollweide, getMollweideLambda0, splitMollweideWrap, getGreatCirclePoints, vectorToRaDecRad, radToMollweide, degToRad } from '../utils/geometryUtils.js';
import { minimalRADifference } from '../utils.js';
import { lightenColor } from './densityColorUtils.js';

/**
 * Load dust cloud data files and convert to point positions.
...
export async function loadCloudPoints(files) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      data.forEach(d => {
        if (d.RA !== undefined && d.DEC !== undefined && d['d (pc)'] !== undefined) {
          const ra = degToRad(d.RA);
...
class CloudDensityGridOverlay {
  ...
  computeCellDensity(cell, points, radius = 10, tolerance = 0) {
    const dArr = points.map(p => cell.tcPos.distanceTo(p.pos));
    dArr.sort((a,b) => a - b);
    let sum = 0;
  ...
  update(points, sceneTC, sceneGlobe, sceneMoll) {
    ...
    this.cubesData.forEach(cell => {
      this.computeCellDensity(cell, points, radius, tolerance);
    });
...
export function updateCloudDensityFilter(points, overlay, sceneTC, sceneGlobe, sceneMoll) {
  if (!overlay) return;
  overlay.update(points, sceneTC, sceneGlobe, sceneMoll);
}
