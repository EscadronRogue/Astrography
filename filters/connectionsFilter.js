...
export function createConnectionLines(stars, pairs, mapType, opacityFactor = 0.5, maxAllowedDistance = null) {
  ...
  pairs.forEach(pair => {
    ...
    if (mapType === 'Globe') {
      ...
      const baseScale = 0.2;
      const sizeA = starA.displaySizeMoll ?? starA.displaySize;
      const sizeB = starB.displaySizeMoll ?? starB.displaySize;
      const offsetA = sizeA * baseScale;
      const offsetB = sizeB * baseScale;
      ...
      segments.forEach(([s1, s2]) => {
        ...
        const materialLine = new THREE.LineBasicMaterial({
          ...
          transparent: true,
          opacity: THREE.MathUtils.lerp(1.0, 0.0, distance / maxDist) * opacityFactor,
    ...
    const baseScale = mapType === 'Mollweide' ? 0.2 : 0.1;
    const sizeA = mapType === 'Mollweide'
      ? (starA.displaySizeMoll ?? starA.displaySize)
      : starA.displaySize;
    const sizeB = mapType === 'Mollweide'
      ? (starB.displaySizeMoll ?? starB.displaySize)
      : starB.displaySize;
    const offsetA = sizeA * baseScale;
    const offsetB = sizeB * baseScale;
