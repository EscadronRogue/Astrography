...
export function applyFilters(allStars) {
  if (!filterForm) {
    ...
    if (!filterForm) {
      return {
        ...
        densityTolerance: 0,
        densityBottomOpacity: 100,
        densityMiddleOpacity: 100,
        densityTopOpacity: 100,
        densityBottomColor: '#0000ff',
        densityMiddleColor: '#00ff00',
        densityTopColor: '#ff0000',
        cloudOpacity: 100,
        starOpacity: 100,
        starNameOpacity: 100,
  ...
  const filters = {
    ...
    densityBottomOpacity: parseFloat(formData.get('density-bottom-opacity')) || 100,
    densityMiddleOpacity: parseFloat(formData.get('density-middle-opacity')) || 100,
    densityTopOpacity: parseFloat(formData.get('density-top-opacity')) || 100,
    densityBottomColor: formData.get('density-bottom-color') || '#0000ff',
    densityMiddleColor: formData.get('density-middle-color') || '#00ff00',
    densityTopColor: formData.get('density-top-color') || '#ff0000',
    ...
    if (
      ...
      if (densityOverlay) {
        densityOverlay.cubesData.forEach(cell => {
          window.trueCoordinatesMap.scene.remove(cell.tcMesh);
          if (window.mollweideMap.scene.children.includes(cell.mollSprite)) {
            window.mollweideMap.scene.remove(cell.mollSprite);
          }
        });
      }
      ...
      densityOverlay.cubesData.forEach(cell => {
        window.trueCoordinatesMap.scene.add(cell.tcMesh);
        window.mollweideMap.scene.add(cell.mollSprite);
      });
    }
    updateDensityFilter(
      allStars,
  ...
  return {
    ...
    densityBottomOpacity: filters.densityBottomOpacity,
    densityMiddleOpacity: filters.densityMiddleOpacity,
    densityTopOpacity: filters.densityTopOpacity,
    densityBottomColor: filters.densityBottomColor,
    densityMiddleColor: filters.densityMiddleColor,
    densityTopColor: filters.densityTopColor,
